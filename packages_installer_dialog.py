...
PLUGIN_ROOT_DIR = os.path.realpath(
    # Change this based on your project structure
...
REQUIREMENTS_PATH = os.path.join(
    PLUGIN_ROOT_DIR, "python_requirements/requirements.txt"
)
...
packages_to_install = [
    PackageToInstall(
        ...
        version=libraries_versions["opencv-python-headless"],
        import_name="cv2",
    ),
]
...
if sys.platform == "linux" or sys.platform == "linux2":
    packages_to_install += [
        PackageToInstall(
            ...
            version=libraries_versions["onnxruntime-gpu"],
...
def are_packages_importable() -> bool:
    try:
        ...
        logging.exception(
            f"Python packages required by the plugin could not be loaded due to the following error:"
        )
        return False
...
class PackagesInstallerDialog(QDialog, FORM_CLASS):
    ...
    def _check_packages_installation_and_log(self) -> bool:
        ...
        if packages_ok:
            msg1 = (
                f"All required packages are importable! You can close this window now!"
            )
            self.log(msg1)
            return True
...
def check_required_packages_and_install_if_necessary(iface):
    ...
    # Open a new window with button "Install packages"
    global dialog
    dialog = PackagesInstallerDialog(iface)
    dialog.setWindowModality(QtCore.Qt.WindowModal)
    dialog.show()
